...
`0.9.0rc7` release draft. The `v0.9.0rc7` tag has not been created,
...
- Use `docs/frontend_visual_smoke_plan.md`,
  ...
  `scripts/smoke_frontend_visual.sh` for optional/manual Playwright visual
  smoke. Visual smoke should cover API connected and API disconnected/demo
  states and must confirm no upload, release, solver, or external LLM controls
  are visible.
...
- Do not upload `0.9.0rc7` to TestPyPI without explicit approval.
...
- Do not create the `v0.9.0rc7` tag until future release-draft checks and
  maintainer approval.
- Do not treat `0.9.0rc7` as a public release; a future `v0.9.0rc7`
