...
| 可选 solver 验证 | Gmsh/Meep/MPB/Optiland micro-benchmark plan | 已记录 pilot 为 `optional_manual_solver_validated` | `validation/solver_validation_micro_benchmarks.json`、已有 2026-05-14 手动报告、2026-05-20 Gmsh-only、Optiland-only 和 Meep-only evidence records、Gmsh/Optiland/Meep review decisions、Meep 和 MPB decision packets 以及 `scripts/run_optional_solver_micro_benchmarks.sh` | 仅显式 opt-in；默认 pytest、smoke、quality gates 和 release gates 不运行 solver。Gmsh 证据只被接受为 mesh-generation smoke，Optiland 证据只被接受为 ray/path smoke，Meep 证据只被接受为 PyMeep/FDTD smoke；都不证明光学正确性。MPB 仍未执行，只准备了 decision packet。 |
...
- Gmsh review record
  ...
  已被 review 接受，只作为 optional manual PyMeep/FDTD smoke evidence。MPB 需要
  `OSA_SOLVER_PYTHON` 和单独批准。Meep decision packet
...
- MPB decision packet
  ...
  路径、未来命令、artifact 和 non-claims；它不授权 MPB 执行。
- 默认不需要外部 LLM。
