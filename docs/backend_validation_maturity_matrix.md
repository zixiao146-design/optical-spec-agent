...
| Optional solver validation | Gmsh/Meep/MPB/Optiland micro-benchmark plan | `optional_manual_solver_validated` for recorded pilots | `validation/solver_validation_micro_benchmarks.json`, existing 2026-05-14 manual reports, the approved Gmsh-only, Optiland-only, and Meep-only 2026-05-20 evidence records, the Gmsh, Optiland, and Meep review decisions, the Meep and MPB decision packets, and `scripts/run_optional_solver_micro_benchmarks.sh` | Explicit opt-in only; default pytest, smoke, release gates, and quality gates do not run solvers. Gmsh evidence is accepted only as mesh-generation smoke, Optiland evidence only as ray/path smoke, and Meep evidence only as PyMeep/FDTD smoke; none proves optical correctness. MPB remains not executed and only has a prepared decision packet. |
...
- The MPB decision packet
  ...
  readiness path, future command, artifacts, and non-claims. It does not
