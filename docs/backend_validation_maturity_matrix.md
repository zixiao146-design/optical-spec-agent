...
| Optional solver validation | Gmsh/Meep/MPB/Optiland micro-benchmark plan | `optional_manual_solver_validated` for recorded pilots | `validation/solver_validation_micro_benchmarks.json`, existing 2026-05-14 manual reports, the approved Gmsh-only and Optiland-only 2026-05-20 evidence records, the Gmsh review decision, and `scripts/run_optional_solver_micro_benchmarks.sh` | Explicit opt-in only; default pytest, smoke, release gates, and quality gates do not run solvers. Gmsh evidence is accepted only as mesh-generation smoke, and Optiland evidence is only ray/path smoke; neither proves optical correctness. |
...
- The Gmsh review record
  ...
  `OSA_SOLVER_PYTHON` and separate approval.
