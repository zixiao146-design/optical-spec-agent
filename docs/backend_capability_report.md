...
6. `requirements_templates`: EN/ZH goal presence, heuristic matching status,
   expected tools, cross-check status, and preview-only status for the seven
   design requirement templates.
7. `blocked_external_actions`: solver, LLM, upload, publication, tag, and
...
- Adapter-native golden cases are fixture checks against local API responses;
  they are not real solver monitor results.
