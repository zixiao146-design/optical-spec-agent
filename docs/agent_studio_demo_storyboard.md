...
| 4a | Material Library | Review preview material candidates. | Local material cards and application suggestions are visible. | `GET /api/materials`, `POST /api/materials/suggest` | "The agent now has a local material memory for design orientation." | Material data is preview-only, not production-grade optical constants. |
...
| 6a | Agent Collaboration | Load nanoparticle agent trace. | SpecAgent, MaterialAgent, GeometryAgent, AdapterAgent, WorkflowAgent, EvidenceAgent, SafetyAgent, and RecommendationAgent are visible. | `POST /api/agent-trace` | "The agent shows how sub-agents coordinate decisions." | Local deterministic trace; no external LLM is called. |
