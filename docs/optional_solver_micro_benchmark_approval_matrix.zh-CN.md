...
| Meep | 2026-05-20 已按 Meep-only 批准执行并通过；记录为 optional manual PyMeep/FDTD smoke evidence | 当前 Python 或 `OSA_SOLVER_PYTHON` 中的 `meep` import probe；推荐 profile 为 `osa-solvers` | `OSA_RUN_OPTIONAL_MEEP_VALIDATION=1` | `scripts/run_optional_meep_validation.sh` | `examples/specs/missing_wavelength_meep_preview.json` | `/tmp/osa-meep-micro-benchmark-output/` artifact 和 `/tmp/osa-meep-micro-benchmark-report.json` | 只验证微型 PyMeep 路径，不是 production FDTD validation；即使探测到 `meep.mpb`，MPB 仍是单独 decision | 2026-05-20 仅批准、执行 Meep，且使用 `OSA_SOLVER_PYTHON=/opt/homebrew/Caskroom/miniforge/base/envs/osa-solvers/bin/python`；未来再次运行需重新批准 | no | no | no |
