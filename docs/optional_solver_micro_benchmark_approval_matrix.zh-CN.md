...
| MPB | Candidate-ready，已有 narrow manual report 和 MPB decision packet；未执行 | 当前 Python 或 `OSA_SOLVER_PYTHON` 中的 `meep.mpb` import probe；如果 Python path 可用，不要求 MPB CLI | `OSA_RUN_OPTIONAL_MPB_VALIDATION=1` | `scripts/run_optional_mpb_validation.sh` | `examples/specs/mpb_preview.json` | `/tmp/osa-mpb-micro-benchmark-output/mpb_validation_result.json`、可选 band summary JSON 和 `/tmp/osa-mpb-micro-benchmark-report.json` | 只做 tiny band path smoke，不是 band convergence evidence，也不是 production-grade MPB validation | yes | no | no | no |
