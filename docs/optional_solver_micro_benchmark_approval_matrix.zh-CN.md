...
| --- | --- | --- | --- | --- | --- | --- | --- | --- | --- | --- | --- |
| Gmsh | 已有窄范围手动报告，候选 ready | 仅 `command -v gmsh` | `OSA_RUN_OPTIONAL_GMSH_VALIDATION=1` | `scripts/run_optional_gmsh_validation.sh` | `examples/specs/gmsh_preview.json` | `/tmp/osa-gmsh-validation/gmsh_preview.msh` 和报告 JSON | 只验证 mesh 路径，不验证光学正确性 | yes | no | no | no |
| Meep | 已有窄范围手动报告，候选 ready | 仅检查 Python module spec `meep` | `OSA_RUN_OPTIONAL_MEEP_VALIDATION=1` | `scripts/run_optional_meep_validation.sh` | `examples/specs/missing_wavelength_meep_preview.json` | `/tmp/osa-meep-validation/meep_validation_result.json` 和报告 JSON | 只验证微型 PyMeep 路径，不是 FDTD 验证 | yes | no | no | no |
| MPB | 已有窄范围手动报告，候选 ready | 仅检查 Python module spec `meep.mpb` | `OSA_RUN_OPTIONAL_MPB_VALIDATION=1` | `scripts/run_optional_mpb_validation.sh` | `examples/specs/mpb_preview.json` | `/tmp/osa-mpb-validation/mpb_validation_result.json` 和报告 JSON | 只验证微型 band 路径，不是 band convergence 证据 | yes | no | no | no |
| Optiland | 已有窄范围手动报告，候选 ready | 检查 Python module spec `optiland`，可选 CLI path | `OSA_RUN_OPTIONAL_OPTILAND_VALIDATION=1` | `scripts/run_optional_optiland_validation.sh` | `examples/specs/optiland_preview.json` | `/tmp/osa-optiland-validation/optiland_validation_result.json` 和报告 JSON | 只验证微型 ray/backend 路径，不是透镜设计验证 | yes | no | no | no |
