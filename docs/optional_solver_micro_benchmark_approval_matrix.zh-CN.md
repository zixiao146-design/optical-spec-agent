...
| Meep | 已准备未来 explicit decision packet；approval 仍 pending，execution 仍 not run | 当前 Python 或 `OSA_SOLVER_PYTHON` 中的 `meep` import probe；推荐 profile 为 `osa-solvers` | `OSA_RUN_OPTIONAL_MEEP_VALIDATION=1` | `scripts/run_optional_meep_validation.sh` | `examples/specs/missing_wavelength_meep_preview.json` | `/tmp/osa-meep-micro-benchmark-output/` artifact 和 `/tmp/osa-meep-micro-benchmark-report.json` | 只验证微型 PyMeep 路径，不是 FDTD 验证；即使探测到 `meep.mpb`，MPB 仍是单独 decision | 需要短语 `I approve running the optional Meep micro-benchmark for optical-spec-agent using OSA_SOLVER_PYTHON=<path>.` | no | no | no |
...
它记录必需的 `OSA_SOLVER_PYTHON` profile、未来命令、artifact 和 non-claims，
同时保持 Meep approval pending 和 execution unauthorized。
