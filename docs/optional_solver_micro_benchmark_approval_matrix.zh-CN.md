...
| MPB | 2026-05-20 已按 MPB-only 批准执行并通过；记录为 optional manual MPB/band-structure smoke evidence，等待单独 review | 当前 Python 或 `OSA_SOLVER_PYTHON` 中的 `meep.mpb` import probe；如果 Python path 可用，不要求 MPB CLI | `OSA_RUN_OPTIONAL_MPB_VALIDATION=1` | `scripts/run_optional_mpb_validation.sh` | `examples/specs/mpb_preview.json` | `/tmp/osa-mpb-micro-benchmark-output/mpb_validation_result.json` 和 `/tmp/osa-mpb-micro-benchmark-report.json` | 只做 tiny band path smoke，不是 band convergence evidence，也不是 production-grade MPB validation | 2026-05-20 仅批准并执行 MPB；未来再次运行需重新批准 | no | no | no |
...
它记录必需的 `OSA_SOLVER_PYTHON` profile、`meep.mpb` import-only readiness
