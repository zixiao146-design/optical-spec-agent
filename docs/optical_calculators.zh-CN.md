...
| 光纤耦合预览 | `POST /api/optics/fiber-coupling` | 包含偏移和倾角因子的标量 Gaussian 模式重叠估计。 |
| Jones 偏振预览 | `POST /api/optics/polarization-jones` | 理想偏振片、波片和偏振态的 Jones 预览。 |
...
- 光纤耦合假设输入光束和光纤模式都是圆对称标量 Gaussian 模式，只包含模式尺寸、
  横向偏移和角度倾斜因子。
- Jones 偏振工具假设相干双分量偏振态和理想偏振片/波片，不包含退偏或矢量电磁传播。
- 波导估计只提供标量 slab 波导方向性判断，不求解矢量本征模。
- 波导厚度扫描和单模范围只是标量方向性判断，不是本征模求解。

## 结果摘要

计算器响应包含 `status`、`result`、`assumptions`、`diagnostics`、`warnings`、
`limitations`、`quality` 和保守安全字段。`quality.quality_level` 当前为
`sanity_checked_preview`，`quality.reference_case` 会在适用时标记本地公式 sanity
check，例如单界面 Fresnel 反射、四分之一波长增透膜、Gaussian Rayleigh range、薄透镜
1:1 成像和平板波导 V-number。扫描类响应包含采样点数量和摘要字段，方便 agent
session 展示“实际计算了什么”，但不会把结果说成生产级物理验证。

## 参考公式

- 薄膜单界面：`R = |(n0 - ns) / (n0 + ns)|^2`。
- 四分之一波长增透膜：`n_coating = sqrt(n0 * ns)`，`d = lambda / (4 * n_coating)`。
- Gaussian 光束：`z_R = pi * w0^2 / lambda`，`w(z_R) = w0 * sqrt(2)`。
- 光纤耦合尺寸失配：
  `eta_w = (2 w_in w_f / (w_in^2 + w_f^2))^2`。
- Jones 线偏振片：`E_out = |a><a| E_in`；理想波片先旋转到快慢轴、施加相位延迟、
  再旋回实验坐标。
- 薄透镜：`1/f = 1/s + 1/s'`。
- ABCD 自由空间：`[[1, d], [0, 1]]`；薄透镜：`[[1, 0], [-1/f, 1]]`。
- 波导 V-number：`V = (2*pi/lambda) * thickness * sqrt(n_core^2 - n_clad^2)`。
