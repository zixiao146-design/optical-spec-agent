...
| Optional solver-backed micro-benchmarks | Existing Gmsh/Meep/MPB/Optiland pilots are now represented in a unified manifest; approved Gmsh-only, Optiland-only, Meep-only, and MPB-only 2026-05-20 micro-benchmarks passed and are recorded as optional manual smoke evidence; Elmer remains deferred | Optional solver-backed checks are not default gates and are not reproducible Level 4 benchmarks; MPB evidence is band-structure path smoke only and is pending separate review | Maintainers may overread manual pilots or decision packets as production validation | Keep `scripts/run_optional_solver_micro_benchmarks.sh` default no-execute and require explicit opt-in variables. |
...
closes only the separately approved Meep run using `OSA_SOLVER_PYTHON`. The MPB
...
evidence pending separate review. Elmer remains deferred.
