...
This path keeps post-rc8 backend engineering separate from future `v0.9.0rc9`,
...
- Keep package version at `0.9.0rc9.dev0` until a release draft is approved.
- Keep `v0.9.0rc9` tag absent.
- Keep `v1.0.0` tag absent.
- Keep PyPI unpublished.
- Keep TestPyPI upload for `0.9.0rc9.dev0` unperformed unless explicitly
  approved.
...
- `project.version` and `__version__` are ready to move from
  `0.9.0rc9.dev0` to `0.9.0rc9`.
- Release draft notes and readiness docs exist.
- `v0.9.0rc9` tag remains absent before creation.
...
Continue v1.0 readiness/backend engineering in `0.9.0rc9.dev0`, then revisit
the `v0.9.0rc9` release-draft decision only after the rc8/rc9 capability gaps are
