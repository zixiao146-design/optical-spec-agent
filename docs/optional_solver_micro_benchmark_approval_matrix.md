...
| Meep | Executed/passed for the approved 2026-05-20 Meep-only run; reviewed and accepted as optional manual PyMeep/FDTD smoke evidence | `meep` import probe through current Python or `OSA_SOLVER_PYTHON`; recommended profile is `osa-solvers` | `OSA_RUN_OPTIONAL_MEEP_VALIDATION=1` | `scripts/run_optional_meep_validation.sh` | `examples/specs/missing_wavelength_meep_preview.json` | `/tmp/osa-meep-micro-benchmark-output/` artifacts and `/tmp/osa-meep-micro-benchmark-report.json` | Tiny PyMeep path smoke only; not production FDTD validation; MPB remains separate even if `meep.mpb` is detected | approved/executed/reviewed for Meep only on 2026-05-20 using `OSA_SOLVER_PYTHON=/opt/homebrew/Caskroom/miniforge/base/envs/osa-solvers/bin/python`; future runs need fresh approval | no | no | no |
| MPB | Executed/passed for the approved 2026-05-20 MPB-only run; recorded as optional manual MPB/band-structure smoke evidence pending separate review | `meep.mpb` import probe through current Python or `OSA_SOLVER_PYTHON`; MPB CLI is not required if the Python path is available | `OSA_RUN_OPTIONAL_MPB_VALIDATION=1` | `scripts/run_optional_mpb_validation.sh` | `examples/specs/mpb_preview.json` | `/tmp/osa-mpb-micro-benchmark-output/mpb_validation_result.json` and `/tmp/osa-mpb-micro-benchmark-report.json` | Tiny band path smoke only; not band convergence evidence or production-grade MPB validation | approved/executed for MPB only on 2026-05-20 using `OSA_SOLVER_PYTHON=/opt/homebrew/Caskroom/miniforge/base/envs/osa-solvers/bin/python`; future runs need fresh approval | no | no | no |
...
The approved MPB run is recorded in
[`optional_solver_approval_records/mpb_micro_benchmark_approval_2026-05-20.md`](optional_solver_approval_records/mpb_micro_benchmark_approval_2026-05-20.md)
and
