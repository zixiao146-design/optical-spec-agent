...
| MPB | Candidate-ready with previous narrow manual report and MPB decision packet; not executed | `meep.mpb` import probe through current Python or `OSA_SOLVER_PYTHON`; MPB CLI is not required if the Python path is available | `OSA_RUN_OPTIONAL_MPB_VALIDATION=1` | `scripts/run_optional_mpb_validation.sh` | `examples/specs/mpb_preview.json` | `/tmp/osa-mpb-micro-benchmark-output/mpb_validation_result.json`, optional band summary JSON, and `/tmp/osa-mpb-micro-benchmark-report.json` | Tiny band path smoke only; not band convergence evidence or production-grade MPB validation | yes | no | no | no |
...
readiness path, future command, artifacts, and non-claims. It does not approve
MPB execution.
