...
| MPB | Executed/passed for the approved 2026-05-20 MPB-only run; reviewed and accepted as optional manual MPB/band-structure smoke evidence | `meep.mpb` import probe through current Python or `OSA_SOLVER_PYTHON`; MPB CLI is not required if the Python path is available | `OSA_RUN_OPTIONAL_MPB_VALIDATION=1` | `scripts/run_optional_mpb_validation.sh` | `examples/specs/mpb_preview.json` | `/tmp/osa-mpb-micro-benchmark-output/mpb_validation_result.json` and `/tmp/osa-mpb-micro-benchmark-report.json` | Tiny band path smoke only; not band convergence evidence or production-grade MPB validation | approved/executed/reviewed for MPB only on 2026-05-20 using `OSA_SOLVER_PYTHON=/opt/homebrew/Caskroom/miniforge/base/envs/osa-solvers/bin/python`; future runs need fresh approval | no | no | no |
