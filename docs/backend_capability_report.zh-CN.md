...
6. `requirements_templates`：七个设计需求模板的中英文 goal、启发式匹配状态、
   预期工具、交叉检查状态和 preview-only 状态。
7. `blocked_external_actions`：solver、LLM、上传、发布、tag、release 默认不执行。
...
- 内部工具现在包含光源/监测器推断、缺失输入诊断、观测量诊断和适配器原生
  ...
  已执行的内部工具显示。
- Adapter-native golden cases 是针对本地 API 响应的 fixture 检查，不是真实
