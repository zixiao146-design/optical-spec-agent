...
当前光学设计方向扩展加入了本地预览 `docs/material_library.zh-CN.md` 材料库、
`examples/optical_design/` 光学设计示例，以及 `docs/sub_agent_architecture.zh-CN.md`
子智能体协作轨迹。Agent Studio 中可以看到 SpecAgent、MaterialAgent、
GeometryAgent、AdapterAgent、WorkflowAgent、EvidenceAgent、SafetyAgent 和
RecommendationAgent 的协作贡献。这些能力仍保持 preview-first：默认不执行外部
