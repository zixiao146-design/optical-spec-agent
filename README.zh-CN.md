...
当前 package version 是 `v0.9.0rc2`。这是已经验证的 GitHub pre-release
candidate，不是 final stable `1.0`。
...
`v0.9.0rc2` tag 和 GitHub pre-release 已由维护者确认后创建，并作为当前
release candidate supersede `v0.9.0rc1`。不要移动 `v0.9.0rc1` 或
...
`v0.9.0rc2` 的 GitHub pre-release 已完成验证。后续 RC 应继续使用
...
`v0.9.0rc2` 是当前 release candidate：

- 不是 final stable `1.0`。
- GitHub pre-release 已创建并验证：draft=false, prerelease=true。
- PyPI 发布需要单独批准。
- Release draft: `docs/github_release_draft_v0.9.0rc2.md`
