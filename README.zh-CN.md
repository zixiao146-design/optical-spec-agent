...
readiness 层新增
`docs/optional_solver_micro_benchmark_approval_matrix.zh-CN.md` approval matrix、
`docs/optional_solver_micro_benchmark_approval_record_template.zh-CN.md` approval record template
`docs/optional_solver_micro_benchmark_execution_packet.zh-CN.md` execution approval packet、
`docs/optional_solver_execution_sequence.zh-CN.md` execution sequence 和
...
`validation/gmsh/gmsh_micro_benchmark_2026-05-20.md` 中汇总。维护者 review
只接受它作为 optional manual mesh-generation smoke evidence；它本身不授权任何后续
solver 执行。单独批准的 2026-05-20 Optiland-only run 记录在
`validation/optiland/optiland_micro_benchmark_2026-05-20.md`，并在
...
PyPI 发布、`0.9.0rc9.dev0` 的 TestPyPI 上传、未来 `v0.9.0rc9` tag 以及
...
当前公开 release candidate 是 `v0.9.0rc8`。当前 `main` 的 development version 是
`0.9.0rc9.dev0`。`v0.9.0rc9` tag 和 GitHub release 尚未创建；`0.9.0rc9.dev0`
...
`v0.9.0rc8` tag 和 GitHub prerelease 已由维护者确认后创建，并作为当前
release candidate supersede `v0.9.0rc7`。不要移动 `v0.9.0rc1`、`v0.9.0rc2`、
`v0.9.0rc3`、`v0.9.0rc4`、`v0.9.0rc5`、`v0.9.0rc6`、`v0.9.0rc7` 或
`v0.9.0rc8` tag。未来 post-release fix 应使用新的 RC tag。PyPI 仍未发布；
TestPyPI 已通过手动 Trusted Publishing 上传 `0.9.0rc6.dev0`，但 `0.9.0rc9.dev0`
...
engineering，不创建 `v0.9.0rc9` tag/release，不授权 PyPI/TestPyPI 上传，
...
`examples/` 目录中的 fixture 是当前 `0.9.0rc9.dev0` main 和公开
`v0.9.0rc8` RC 线的本地示例证据：
