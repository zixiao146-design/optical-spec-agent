"""Recorded Meep manual validation evidence checks."""
...
REPORT = ROOT / "validation" / "meep" / "meep_validation_pilot_2026-05-14.md"


def test_meep_manual_validation_report_bounds_level3_claims():
    assert REPORT.exists()
    text = REPORT.read_text(encoding="utf-8")
    assert "Project version: 0.9.0rc5.dev0" in text
    assert "Adapter family: meep" in text
    assert "Solver name: Meep / PyMeep" in text
    ...
    assert "This does not make Meep a default dependency" in text


def test_meep_report_and_maturity_model_are_consistent():
    maturity = (ROOT / "docs" / "adapter_maturity_model.md").read_text(encoding="utf-8")
    readiness = (ROOT / "docs" / "meep_level3_readiness.md").read_text(encoding="utf-8")
    assert "| Meep | Level 3" in maturity
    assert "validation/meep/meep_validation_pilot_2026-05-14.md" in maturity
    assert "Meep current maturity: Level 3" in readiness
    assert "Level 3 achieved: yes" in readiness
    assert "Default pytest does not run Meep" in readiness
