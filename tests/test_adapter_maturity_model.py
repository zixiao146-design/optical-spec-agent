...
def test_adapter_maturity_model_exists_and_bounds_claims():
    ...
    assert "| Elmer | Level 2" in text
    assert "Level-3-ready" in text
    ...
    assert "docs/elmer_level3_readiness.md" in text
    assert "scripts/run_optional_elmer_validation.sh" in text
    ...
    assert "Level 3 is not achieved" in text
