...
def test_pyproject_core_packaging_metadata_present():
    ...
    assert project["version"] == "0.9.0rc9.dev0"
...
def test_packaging_gate_docs_and_pypi_decision_are_present():
    ...
    assert "TestPyPI upload for 0.9.0rc9.dev0: not performed" in packaging_gate
    assert "Current main development version: `0.9.0rc9.dev0`" in packaging_gate
    assert "Current public prerelease: `v0.9.0rc8`" in packaging_gate
    assert "`v0.9.0rc9` tag: not created" in packaging_gate
    ...
    assert "TestPyPI upload for `0.9.0rc9.dev0`: not performed" in pypi_decision
    assert "Current main development version: `0.9.0rc9.dev0`" in pypi_decision
    assert "explicit maintainer approval" in pypi_decision
    assert "TestPyPI upload approval status for 0.9.0rc9.dev0: pending" in pypi_decision
