...
def test_pyproject_core_packaging_metadata_present():
    ...
    assert project["version"] == "0.9.0rc8"
...
def test_packaging_gate_docs_and_pypi_decision_are_present():
    ...
    assert "TestPyPI upload for 0.9.0rc8: not performed" in packaging_gate
    assert "Current main release draft: `0.9.0rc8`" in packaging_gate
    ...
    assert "TestPyPI upload for `0.9.0rc8`: not performed" in pypi_decision
    assert "Current main release draft: `0.9.0rc8`" in pypi_decision
    assert "explicit maintainer approval" in pypi_decision
    assert "TestPyPI upload approval status for 0.9.0rc8: pending" in pypi_decision
