...
class SpecValidator:
        ...
        if phys_sys == "nanoparticle_on_film":
            ...
            for path in [
                ...
                if self._is_missing(spec, path):
                    warnings.append(
                        f"物理系统为 nanoparticle_on_film，建议补充 {path}"
                    )
    ...
    def _check_postprocess_rules(
        ...
        # If FWHM or T2 extraction → need spectrum-type output
        ...
        if any(t in needs_spectrum for t in target_types):
            ...
            if not any(o in spectrum_types for o in obs_list):
                warnings.append(
                    "后处理包含 FWHM/T2/拟合 目标，但 output_observables 缺少 spectrum 类输出"
                )
